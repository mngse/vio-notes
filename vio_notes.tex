...
\State \textbf{Upon arrival of new keyframe} $x_k$:

\Statex \textbf{1. State Initialization}
\State Propagate $x_{k-1}$ using $\mathcal{S}_{k-1 \to k}$ to obtain initial guess $\check{x}_k$
...
\For{each observation $\mathbf{u}_{i\ell} \in \mathcal{U}_k$}
    \State $\mathbf{r}^{cam}_{i\ell} = \mathbf{u}_{i\ell} - \pi(x_i, \mathbf{P}_\ell)$ \Comment{Difference from projection model}
\EndFor

\Statex \textbf{4. Inertial Preintegration and Residuals}
\For{each consecutive pair $(i, j) \in \mathcal{X}$}
    \State Compute preintegrated $\Delta \hat{R}_{ij}, \Delta \hat{v}_{ij}, \Delta \hat{p}_{ij}$ from $\mathcal{S}_{i \to j}$
...
\State $\hat{\mathcal{X}} \gets \arg\min_{\mathcal{X}} J(\mathcal{X})$ \Comment{Solve via least-squares}

\Statex \textbf{6. Output}
\State \Return Latest optimized state $\hat{x}_k$
\end{algorithmic}
\end{algorithm}
