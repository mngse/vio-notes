...
\end{equation}
with
\(\mathbf{R}_i\in \SO(3)\), \(\mathbf{p}_i,\mathbf{v}_i\in\R^3\), and gyro/accelerometer biases
...
\paragraph{Comment.}
The camera factor penalizes the mismatch between the measured image point and the point predicted by the current state and landmark estimate.
...
\section{Two distinct roles of the IMU}
...
This is only an initialization step for the optimizer.

\paragraph{Comment.}
This is where one informally says that the IMU ``predicts the next state.''

\subsection{Role 2: building IMU factors in the optimization window}
For every consecutive pair of states \((i,j)\) currently in the active optimization window, the raw IMU samples between \(t_i\) and \(t_j\) are preintegrated into one IMU factor.
Therefore preintegration is not only used for the newest interval: it exists for all active IMU intervals in the window.

\paragraph{Comment.}
This is the main clarification: the optimization uses stored IMU factors on all intervals in the current window, not only on the newest state.
...
\end{equation}
The newest state \(\mathbf{x}_5\) is initialized by IMU propagation from \(\mathbf{x}_4\), but the optimization uses all active IMU factors in the current window.
...
Window before new frame:
  X_old = {x_{k-m}, ..., x_{k-1}}

When new keyframe x_k arrives:

1. Initialize x_k
   - propagate x_{k-1} using raw IMU from t_{k-1} to t_k
   - this gives an initial guess for x_k

2. Form the new window
   X = {x_{k-m+1}, ..., x_{k-1}, x_k}

3. Build / store IMU factors
   For each consecutive interval (i,j) in the active window:
     - use raw IMU samples between t_i and t_j
     - preintegrate them to get DeltaR_hat_ij, DeltaV_hat_ij, DeltaP_hat_ij
     - store covariance and bias Jacobians

4. Optimize all variables in X together
   - camera factors constrain states and landmarks
   - IMU factors constrain consecutive states
   - all states in the active window can be refined

5. Output the latest estimate x_k
   - keep the optimized window for the next iteration
\end{verbatim}
...
\begin{tikzpicture}[>=Latex]
  \tikzstyle{block}=[draw,rounded corners,minimum width=32mm,minimum height=10mm,align=center]

  % Camera row
  \node[block] (cammeas) at (0,0) {camera measurements\\$\mathbf{u}_{i\ell}$};
  \node[block] (cammodel) at (4.5,0) {projection model\\from $\mathbf{x}_i,\mathbf{P}_\ell$};
  \node[block] (camres) at (9,0) {camera residual\\$\mathbf{r}^{cam}_{i\ell}$};

  % IMU row
  \node[block] (imumeas) at (0,-2.2) {raw IMU samples\\$(\omega_m,a_m)$};
  \node[block] (preint) at (4.5,-2.2) {preintegration\\$\widehat{\Delta R},\widehat{\Delta v},\widehat{\Delta p}$};
  \node[block] (imustate) at (9,-2.2) {state-implied motion\\from $\mathbf{x}_i,\mathbf{x}_j$};
  \node[block] (imures) at (13.5,-2.2) {IMU residual\\$\mathbf{r}^{imu}_{ij}$};

  % Bottom row
  \node[block] (cost) at (9,-4.6) {weighted least-squares cost\\$J(\mathbf{X})$};
  \node[block] (opt) at (13.5,-4.6) {optimizer};
  \node[block] (out) at (17.5,-4.6) {updated states\\$\hat{\mathbf{X}}$};
  ...
  \draw[->] (imumeas) -- (preint);
  \draw[->] (preint) -- (imures);
  \draw[->] (imustate) -- (imures);

  % Toward cost
  \draw[->] (camres) |- (cost.north);
  \draw[->] (imures) -- (cost);
